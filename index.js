...
function findIndexByProperty(arr, name) {
    ...
    let str = name;
    const foundIndex = arr.findIndex(({ name }) => name === str);
...
function getPositiveArrayValues(arr) {
    ...
    return positiveArrayValues.filter((elem) => elem > 0);
...
function getLastElementsSumAndPrint(arr, n) {
    ...
    const sum = sliceArr.reduceRight((acc, cur) => acc + cur, 0);
...
function capitalizeWords(arr) {
    ...
    const capitalizedArray = arr.map((elem, index) => {
...
function calculateTotalPrice(arr) {
    ...
    const totalPrice = arr.reduce((num, user) => num + user.price, 0);
    return totalPrice;
